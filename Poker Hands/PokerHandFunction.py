def findPokerHand(hand):
    ...
    for card in hand:
        if len(card) == 2: # for every card other than 10
            rank = card[0]
            suit = card[1]
        else:
            rank = card[0:2] # for the number 10 as it increases one digit
            suit = card[2]
        if rank == "A":
            rank = 14
        elif rank == "K":
    ...
    if suits.count(suits[0]) == 5: # Check for Flush
        ...
        elif all(sortedRanks[i] == sortedRanks[i - 1] + 1 for i in range(1, len(sortedRanks))): # checks for straight
    ...
    output = pokerHandRanks[max(possibleRanks)] # gives the best hand as the output, according to the number associated to each kind
    print(hand, output)
    return output
